...
from models.song_model import Song, Track, Note, NoteType, EffectType
...
class MidiWriter:
    ...
    def _process_track(self, track: Track) -> mido.MidiTrack:
        ...
        for ch in channels:
            midi_track.append(mido.Message('program_change', program=current_program, channel=ch, time=0))
        ...
        # This is a bit complex: We need to flatten the IR (Measures -> Beats -> Notes) into a list of events.
        events = []
        ...
        # Read PyGuitarPro docs/source: Beat.duration.value is an integer? No, it's a Duration object.
        ...
        for measure in track.measures:
            ...
            for beat in measure.beats:
                ...
                # PyGuitarPro: beat.start is "Start time of the beat". relative to measure? or song?
                ...
                # beat.start_time is already absolute ticks from the parser (using 960 TPQ)
                ...
                for note in beat.notes:
                    ...
                    vel = note.velocity
                    # Duration
                    # In the parser, beat.duration is already calculated in ticks (e.g. 960 for quarter note)
                    ...
                    if note.midi_number is not None:
                        ...
                        midi_note = note.fret + track.tuning[note.string-1] if not track.is_percussion else note.fret # Fallback

                    events.append({'time': start_abs, 'type': 'note_on', 'note': midi_note, 'velocity': vel, 'channel': channel})
                    events.append({'time': end_abs, 'type': 'note_off', 'note': midi_note, 'velocity': 0, 'channel': channel})
        ...
        for ev in events:
            ...
            if dt < 0: dt = 0 # Safety
            midi_track.append(mido.Message(ev['type'], note=ev['note'], velocity=ev['velocity'], channel=ev['channel'], time=dt))
            last_time = ev['time']
